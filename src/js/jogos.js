document.addEventListener('DOMContentLoaded', function () {
    ...
    function criarModalResumoHTML(jogo) {
        // Função para gerar a representação dos gols (nome do jogador, número e bolas)
        function gerarGolsHTML(jogador) {
            if (jogador.gols > 0) {
                const bolas = '⚽'.repeat(jogador.gols); // Use um emoji de bola
                return `<span>${jogador.nome}: ${jogador.gols} ${bolas}</span>`;
            }
            return '';
        }
                            ...
                            <div class="match-container d-flex align-items-center justify-content-between w-100 mb-3">
                             ...
                             <div class="text-center d-flex justify-content-around ">
                                    <div>
                                        ...
                                        .map(jogador => `${gerarGolsHTML(jogador)}`) // Apenas mostra a quantidade de gols e a bola
                                        .join(', ') || ''
                                         }
                                        </p>
                                     </div>
